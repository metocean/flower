...
def satisfies_search_terms(task, search_terms):
    ...
    terms = [
        ...
        any_value_search_term and any_value_search_term in '|'.join(filter(None,
            [task.name, task.uuid, task.state, getattr(task.worker,'hostname',None), str(task.args), str(task.kwargs), str(task.result)])),
        ...
        args_search_terms and task_args_contains_search_args(str(task.args), args_search_terms)
    ]
    return any(terms)
